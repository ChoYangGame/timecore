...
| 4시대 근접 몹 걷기 각 6프레임 (랩터 · 기사 · 로봇 · 스토커) + 오브젝트 3종 (모래시계 · 총알 · 보스탄) + 시대별 포탑·탄 각 4종 | 2026-08-09~10 디자인 담당이 전달 (기사: 3×2 시트 1920×720 보라 크로마키 / 랩터·로봇·스토커: 낱장 1280×720 **투명 PNG**) | 팀 자체 제작(생성형 AI) — 정지 원화 **Codex GPT-5.6 Sol (high)**, 걷기 프레임은 **Higgsfield Seedance 2.0 Fast** 4초 영상에서 선별 | 근접 몹 애니메이션 (`Assets/Art/Enemies/` — raptor 420 KB / knight 319 KB / robot 403 KB / stalker 322 KB, 전부 세로 192px = 1유닛) + `Assets/Art/Objects/` 모래시계·총알·보스탄·시대별 포탑 4 + 포탑탄 4 |
| 근접무기 시간의 홀(Energy Scepter) 휘두르기 10프레임 | 2026-08-10 디자인 담당이 전달 (1280x720 **투명 PNG**) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** | 칼잡이 무기 그림 겸 히트박스 (`Assets/Art/Player/scepter_swing_0~9.png` — 208 KB) |
| EXP 구슬 (결정 구체 1장) | 2026-08-10 디자인 담당이 전달 (1254x1254 **투명 PNG**) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** (낱장이지만 영상 단계를 거쳐 가장 안정된 프레임을 선별) | 경험치 오브 (`Assets/Art/Objects/exp_orb.png` — 31 KB) |
| 플레이어 호버 6프레임 + 거너폼 총(Chrono Blaster) 발사 8프레임 | 2026-08-10 디자인 담당이 전달 (낱장 1280×720 **투명 PNG**) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** | 플레이어 캐릭터·거너 무기 (`Assets/Art/Player/` — 호버 430 KB / 총 294 KB) |
| 4시대 보스 애니메이션 (티렉스 6 · 기사 6 · 전차 6 · 시계 코어 8, 총 26프레임) | 2026-08-10 디자인 담당이 전달 (낱장 1280×720 **투명 PNG**) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** | 보스 애니메이션 (`Assets/Art/Bosses/` — primitive 769 KB / medieval 580 KB / modern 939 KB / future 1301 KB, 전부 세로 256px = 1유닛) |
| 시대별 BGM 4곡 (원시 「돌의 첫 박동」 · 중세 「십자검 성채」 · 현대 「통제 구역 이상」 · 미래 「크로노 가디언」) | 2026-08-10 디자인 담당이 **Suno (Pro 구독)로 직접 생성** — MP3 스테레오, 원본 합계 6.4 MB | 팀 자체 제작(생성형 AI). Suno Pro 구독자의 상업적 이용 권리에 따름 | 시대별 배경음악 (`Assets/Resources/BGM/` — 모노·Vorbis q0.4 재인코딩 후 합계 **2.0 MB**, 각 51~89초 루프) |
| 메인 화면 배경 아트 (시간관리국 홀) | 2026-08-10 디자인 담당이 전달 (1672×941 PNG, 원본 1.8 MB) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** (낱장이지만 영상 단계를 거쳐 프레임 선별) | 타이틀·직업 선택 화면 배경 (`Assets/Art/UI/title_bg.png` — 1280×720로 축소, DXT1 **450 KB**) |
| 장판 아트 2종 (감속 장판 · 매지션 공격 장판) | 2026-08-10 디자인 담당이 전달 (1024×1024 / 1536×1024 투명 PNG) | 팀 자체 제작(생성형 AI) — **Codex GPT-5.6 Sol (high)** + **Higgsfield Seedance 2.0 Fast** (낱장이지만 영상 단계를 거쳐 프레임 선별) | 시간 감속 지대 · 매지션 지대 (`Assets/Resources/FX/slow_field.png` 512² DXT5 256 KB / `timestop_field.png` 768×512 DXT5 384 KB) |
...
> 상세 명세는 `docs/시간관리국_AI_활용_기술서.md` 09절.

| 항목 | 생성 도구/모델 | 프롬프트 | 후처리 |
|---|---|---|---|
| 4시대 맵 / 플레이어 / 몹 4종 / 보스 4종 / 무기 3종 / 장판 2종 / 메인 배경 | **Codex — GPT-5.6 Sol (reasoning high)** 로 정지 원화 생성·편집 + **Higgsfield — Seedance 2.0 Fast** 로 4초 image-to-video — 디자인 담당 | 정체성 고정 + `#00FF00` 단색 배경 + 그림자·블룸·궤적 금지 / 영상은 카메라 고정·제자리·포즈를 초 단위로 배치 | 97프레임 전개 → 6~10프레임 선별 → 크로마키 소프트 매트·디스필 → RGBA 규격 검사 → Unity 임포트 시 크기·피벗 보정 |
| 시대별 BGM 4곡 | **Suno (Pro 구독)** — 디자인 담당 | 시대별 정체성(원시=타악 / 중세=현악·합창 / 현대=긴장 전자음 / 미래=신스), 루프 전제 | 모노 강제 + Vorbis q0.4 재인코딩(6.4MB → 2.0MB), 시대 전환 시 1초 크로스페이드 |
| 효과음 16종 | 생성형 AI 아님 — **Claude Code가 작성한 파형 합성 코드**(사인·사각·톱니·노이즈 + 포락선) | "버튼·피격·발사·보스 등장·적 피격·EXP 획득 등을 만들어서 넣어줘. 더 필요하면 알아서 판단" | 클립별 목표 피크 정규화, 끝 4ms 페이드로 클릭 제거. 22050Hz 모노, 빌드 증가 0바이트 |
| 이펙트 텍스처 6종 | 생성형 AI 아님 — **Claude Code가 작성한 런타임 생성 코드** | "에셋스토어 대신 코드로 굽는 편이 용량·라이선스에서 이득" 판단 후 진행 | 전부 64×64 / PPU 64로 월드 bounds 정확히 1×1. 빌드 증가 0바이트 |
| 게임 코드 전반 | Claude Code (Sonnet 5 → Opus 5) + Unity MCP | 기술서 03.3의 4가지 프롬프트 패턴 (사양 못박기 / 하지 말 것 명시 / 검증 절차 포함 / 감각 지적 후 원인 규명) | Play 모드 수치 실측 + 화면 캡처 + 콘솔 에러 0건 확인 후에만 채택 |

---

## 팀원별 담당 (제출물 5번 원재료)

> 정리본: `docs/TEAM_ROLES.md` (제출용 팀 소개 문서)

| 팀원 | 역할 | 실제 구현 영역 | 협업 방식 |
|---|---|---|---|
| | 개발 | 게임 설계·스코프 관리 / 코어 루프(이동·자동사격·웨이브·경험치·증강 23종·직업 3종) / 시대 전환 구조와 보스 12패턴·시대 기믹 / AI 디렉터(규칙 기반 적응형 난이도) / UI·HUD·TMP 한글 서브셋 / 온라인 랭킹(Vercel 서버리스 + Upstash Redis) / 아트·사운드 게임 내 통합 및 히트박스 정합 / 효과음 16종 코드 합성 / WebGL 빌드·배포·용량 최적화 / Play 모드 실측 검증과 문서화 | 받은 아트에 맞춰 판정·연출을 다시 맞추는 방식으로 통합. 매 세션 AI_LOG 기록 |
| | 디자인 | 아트 방향 설정(시간관리국 세계관 · 4시대 콘셉트) / 생성형 AI 기반 2D 에셋 제작(맵·플레이어·몹·보스·무기·이펙트) / 프롬프트 설계(시점·비율·광원·그림자) / AI 영상에서 프레임 선별해 스프라이트 애니메이션 구성 / 무기별 모션(반동·회전·휘두르기) / 투명 PNG 정리, 크기·기준점·프레임 순서 규약화 / 시대별 BGM 4곡 | 낱장 투명 PNG + 프레임 순서·권장 속도를 함께 전달 |
...
### 2026-08-10 (D-0) | 매지션 전면 개편: 궤도 코어 → 지속 피해 지대

- **도구**: Claude Code (Opus 5) / Unity MCP (판정 실측 3세트 · 이동속도 실측 · 캡처)
- **작업**: 매지션을 몸 주위를 도는 코어에서 **몸을 중심으로 깔리는 지대**로 바꾸고, 전용 증강 6종을 전부 새 동작에 맞게 갈아끼웠다.
- **프롬프트 원문**: "지금 메이지 폼을 전격 수정할거야 원으로 돌아가는게 아니라 장판 형태로 도트데미지 들어가듯이 장판 안에 들어오면 적에게 데미지를 입히는 방식으로 바꿀게 그에 맞게 메이지폼 증강도 수정해줘"
- **클래스 이름을 안전하게 바꾼 방법**: `OrbitWeapon` → `FieldWeapon`으로 파일과 클래스를 함께 옮기되 **`.meta`를 같이 이동해 GUID를 보존**했다(`git mv` 두 번). 씬의 컴포넌트는 파일 GUID로 스크립트를 찾으므로 참조가 끊기지 않는다 — 실제로 Play 없이 `GetComponent<FieldWeapon>()`이 기존 인스턴스를 그대로 집어내는 것을 확인했다. `AugmentType`도 이름만 갈고 **순서는 그대로 뒀다**(위치가 밀리면 기존 .asset의 type 인덱스가 전부 어긋난다).
- **설계 판단과 근거**: 판정은 콜라이더 없이 주기당 1회 거리 비교다(적 40마리면 비교 40번). 적별 체류 시간 표를 두지 않은 것도 같은 이유 — 표를 들면 상태가 늘고 지대를 드나들 때 꼬인다. **감속은 RiftZone과 같은 정적 조회 방식**(`FieldWeapon.SlowMultiplierAt`)으로 붙여 Enemy가 매 프레임 물어보게 했다. 이벤트도 진입/이탈 처리도 없어 꼬일 데가 없고, 감속 지대와 그냥 곱해진다. **보이는 링이 곧 판정 경계**가 되도록 링 스프라이트를 판정 반경에서 역산했다 — 어제 참격·코어에서 세운 원칙을 그대로 이었다.
- **증강 6종**: 출력 증폭(피해) / 지대 증대(반경) / 고속 전개(주기) / 시간 정체(감속) / 중심 붕괴(최근접 1기 추가 피해 — 광역이라 단일 대상이 약한 구멍을 메운다) / 주위 파동(바깥 링 절반 피해, 1회성).
- **검증 방법**: 판정 경계 5점(1.0~5.0) **5/5 일치**. 중심 붕괴 배율 1.76(기대 1.8), 바깥 링 0.50(정확). 감속은 정적 조회 지대 안 0.608 / 밖 1.000, 실이동은 무감속이면 27유닛 갈 시간에 9.82유닛만 이동. 세 직업 풀 모두 11종 대칭 유지.
- **측정에서 두 번 헛짚음**: 처음엔 적이 이미 플레이어에 도달한 뒤라 평균 속도가 희석됐고(9유닛/14.6초), 다음엔 거리를 9로 잡아 또 도착했다. 거리 120으로 늘려서야 이동 중 구간을 쟀다.
- **남은 것**: README에 직업 3종 설명이 아직 없다. `위상 이동` 설명("피격 시 짧은 무적")도 기본 무적 0.3초가 생겨 이제는 "무적 연장"이 정확하다.
- **담당**: 개발

### 2026-08-10 (D-0) | 보스 4종 기믹을 확정 아트 기준으로 전면 재설계

- **도구**: Claude Code (Opus 5) / Unity MCP (프리팹 주입 · 4보스 동시 소환 검증)
- **작업**: 디자인 확정본 이미지를 보고 보스 4종의 패턴을 다시 짰다. 12패턴(4×3) → **16패턴(4×4)**.
- **프롬프트 원문**: "이 이미지에 보이는 근거리 몹과 보스몹 이게 지금 몬스터 디자인인데 지금 보스몹 4개 디자인을 정말 유심하게 보고 이에 맞는 보스 스킬(기믹)을 다시 다 재설계해서 적용해줘"
- **아트 → 패턴 대응**: (1) **T-Rex** 거대한 턱·몸을 타고 흐르는 청록 균열·긴 꼬리 → 포식 돌진 / **균열 분출**(Fissure를 보스 중심 방사 — 몸의 균열이 땅으로 뻗는 그림) / 꼬리 휩쓸기 / 추격. (2) **데스나이트** 대검·보라 망령 화염·망토 → **대검 강타**(Cross 배치가 내려찍기로 읽힌다) / 망령 소환 / 처형 돌격 / 화염 낙인. (3) **전차** 굵은 주포 1문·상부 미사일 포드 2기·무한궤도 → **주포 직사**(예고 1.4초·피해 22로 가장 무겁게) / 미사일 일제사(유도) / 기관포 소사 / 캐터필러 돌진. (4) **모래시계** 부유 파편 링·다이아 위성 4기 → **위성 레이저**(빔 4갈래 방사) / 시간 정지 영역 / 위상 도약 / 궤도 파편.
- **뺀 것 두 가지(아트 근거)**: 중세 보스의 `심문의 창`(석궁 연사)을 삭제했다 — 확정 아트에서 **석궁은 원거리 몹의 것**이고 보스는 대검을 든다. 미래 보스에는 **돌진을 넣지 않았다** — 떠 있는 구조물이라 몸으로 밀고 들어오면 아트와 어긋난다. 대신 위상 도약이 그 자리를 맡는다.
- **설계 판단과 근거**: 패턴 시스템이 데이터 주도(PatternKind + 직렬화 파라미터)라 **코드를 한 줄도 고치지 않고** 프리팹 주입만으로 끝냈다. 시대 기믹과 같은 beamStyle을 쓰게 해 보스 패턴도 그 시대 것으로 읽히게 유지했다.
- **검증 방법**: 네 시대 세트를 **동시에 소환**해 40초간 방치 — 4기가 각기 다른 패턴으로 순환하는 것을 두 시점에 확인(꼬리 휩쓸기/처형 돌격/기관포 소사/궤도 파편 → 포식 돌진/망령 소환/미사일 일제사/위성 레이저), 예외 0건, 조각 풀 3/160으로 여유.
- **확인한 것**: 패턴 `label`은 `CurrentPatternLabel`에 담기지만 **읽는 곳이 없어** 화면에 안 뜬다(보스 이름은 `EraConfig.bossName`). 새 패턴명 28자가 폰트 서브셋에 없지만 굽기가 불필요한 이유다 — 짐작하지 않고 사용처를 먼저 확인했다.
- **담당**: 개발(기믹) / 디자인(보스 4종 원화)

### 2026-08-10 (D-0) | 보스 서명 기믹 4종 신규 구현 (재배치가 아니라 새 기술)

- **도구**: Claude Code (Opus 5) / Unity MCP (패턴별 단독 피해 실측 · 캡처)
- **작업**: 보스마다 **없던 패턴**을 새로 만들었다. `PatternKind` 8종 → 12종.
- **프롬프트 원문**: "지금 보스 패턴 수정된거 맞아? 그냥 끼워 맞추지 말고 확실하게 바꿔서 패턴에 맞게 수정해주고 이팩트도 수정해줘야지"
- **지적이 정확했다**: 직전 작업은 기존 8종을 **재배치하고 수치만 조정한 것**이었다. "꼬리 휩쓸기"라 이름 붙인 것도 실제로는 다른 보스와 똑같은 방사 탄막이었고 이펙트는 손대지 않았다. 끼워 맞춘 게 맞다.
- **새로 만든 4종과 "피하는 방법"이 다른 이유**: (1) **TailSweep** — 부채꼴이 보스 주위를 훑는다. 탄막이 "틈으로 비집는 것"이라면 이건 **반대쪽으로 도는 것**이다. 근접 보스에 붙는 위험을 만든다. (2) **ShockSlam** — 퍼지는 링. 이 게임의 다른 기술이 전부 "어느 쪽으로"인데 이것만 **"어느 거리에"** 다. 붙어 있던 근접 직업에게 한 번은 판단을 강요한다. (3) **AimedCannon** — 조준선이 플레이어를 따라 돌다 **고정**된다. 기존 빔은 화면 축에 나란해 "어디서 오는지"가 없었는데, 이건 보스에서 뻗는 한 줄이고 **고정된 뒤에 움직여야** 산다. (4) **OrbitSweep** — 갈래가 회전한다. 고정 4갈래는 사이에 서면 무시할 수 있었지만 이건 안전지대가 돌아온다.
- **이펙트도 판정에서 역산했다**: 꼬리 초승달은 처음에 `radius*1.15` 눈대중이라 그림이 사거리 밖으로 삐져나왔다(캡처로 확인). 칼잡이 참격과 같은 규칙 — 호의 현 길이에서 스프라이트 크기를 구해 바깥 끝을 사거리에 맞췄다. 이 비율 상수는 `BladeWeapon` private에서 **`FxSprites`로 옮겨** 두 곳이 같은 값을 쓰게 했다(스프라이트를 설명하는 값이므로 스프라이트 쪽이 제자리다).
- **검증 방법**: 보스를 이동·접촉 0으로 세우고 패턴을 하나씩만 반복시켜 순수 피해를 쟀다. 꼬리 160(=20×8), 충격파 154(=22×7), 주포 104(=26×4), 선회 192(=16×12) — **전부 나머지 0**. 연출도 캡처로 확인(초승달 원형 훑기 / 조각 38개의 퍼지는 링 / 부챗살 레이저 8갈래).
- **오진 1건**: 처음엔 "꼬리 판정도 연출도 안 걸린다"고 판단했는데 틀렸다. 첫 타격이 k≈0.40에서 일어나 내가 심은 로그 창(k 0.45~0.55)보다 **먼저** 발생했고, 누적피해 52도 계산해 보니 빔 16×2 + 꼬리 20이었다. 표본을 잘못 잡고 결론을 낸 것이다.
- **작업 함정**: Play 중에 프리팹을 저장하면 스크립트 리로드가 걸려 `EffectSystem`의 static 인스턴스가 날아간다(씬 오브젝트는 살아 있지만 Awake가 다시 안 돎). 이 상태에서는 **모든 연출이 조용히 안 나와** 검증이 무의미해진다. `PoolSize`가 0인지 먼저 보고 판단해야 한다.
- **담당**: 개발

### 2026-08-10 (D-0) | 중세 시대 근접 몹(해골 기사) 걷기 애니메이션 적용

- **도구**: Claude Code (Opus 5) / Unity MCP (크로마키 처리 · Play 캡처)
- **작업**: 3×2 시트로 받은 기사 6프레임을 잘라 배경을 빼고 중세 시대 근접 몹에 물렸다.
- **프롬프트 원문**: "자 이게 중세시대 몬스터 스프라이트인데 이번엔 6개가 합쳐진 버전으로 줬어 적용시켜봐라"
- **공룡 때와 다른 점**: 공룡은 알파가 이미 있었고(파란 RGB만 남은 상태) 기사는 **알파 없이 보라 단색 배경**(74,33,100)이라 실제로 빼내야 했다.
- **설계 판단과 근거**: 색 임계값만으로 지우지 않고 **셀 테두리에서 flood fill** 했다. 키색과의 거리 40~90 구간이 표본의 11.6%였는데 여기에 갑옷의 어두운 부분이 섞여 있어, 거리로만 자르면 몸통에 구멍이 뚫린다. 테두리에서 번져 들어가면 가운데 어두운 갑옷은 밝은 외곽선에 막혀 도달하지 못하므로 안전하다. **기사의 보라색 눈**도 같은 이유로 살았다 — 배경과 색이 가깝지만 몸 안쪽이라 도달 불가다. 축소는 공룡과 같은 알파 가중 평균이고, 세로 192px = 1유닛 기준도 공룡과 맞춰 두 시대 몹의 화면 크기가 일관된다.
- **검증 방법**: 6프레임 공통 크롭(324×311) → 200×192, 합계 319 KB. Play에서 6장을 늘어놓고 윗줄 원본 / 아랫줄 flipX로 캡처해 비교 — 방패가 좌우로 뒤집히고 배경 잔여물·보라 테두리 없음. 스프라이트 1.04×1.00 유닛(적 콜라이더 1×1과 일치).
- **프레임 순서**: 시트를 좌→우, 위→아래(행 우선)로 읽었다. 공룡 때 파일명 순서가 맞았던 것과 같은 가정이라 실제 사이클과 다르면 배열만 재정렬하면 된다.
- **담당**: 개발(반입·구현) / 디자인(기사 6프레임)

### 2026-08-10 (D-0) | 중세 몹 애니메이션 미작동 수정 + 크로마키 한계 확인

- **도구**: Claude Code (Opus 5) / Unity MCP (프리팹 조사 · 색 분포 실측 · 실스폰 검증)
- **작업**: 중세 기사가 애니메이션도 좌우 반전도 안 되던 원인을 잡고, 다리 사이에 남은 크로마키를 제거했다.
- **프롬프트 원문**: "걷는 애니메이션 적용이 안됐고 좌 우 왔다갔다도 안되고 외곽선은 잘 따졌는데 안에 다리사이에 낑긴 크로마키가 안빠졌네 이걸 우리가 하는게 맞을까 디자이너한테 6개 이미지 파일로 크로마키 투명화 해서 달라고 하는게 맞을까"
- **원인 (아트와 무관한 우리 버그)**: **시대마다 적 프리팹이 다르다.** 원시만 `Enemy.prefab`이고 중세·현대·미래는 `MedievalEnemy.prefab`을 공유하는데, 공룡 작업 때 `SpriteWalkAnimator`를 원시 프리팹에만 붙였다. 컴포넌트가 없으니 프레임 순환도 flipX도 통째로 죽어 있었다. 공룡이 멀쩡해서 배선이 맞다고 착각한 것이다. 앞으로 프리팹이 늘어도 놓치지 않게, 시대 설정이 참조하는 프리팹을 전부 훑어 붙이는 방식으로 처리했다.
- **검증을 잘못했던 것**: 기사 반입 직후 검증에서 6장을 늘어놓고 봤는데 그때 **애니메이터를 꺼두고**(`an.enabled = false`) 프레임을 수동으로 물렸다. 정지 그림만 확인한 셈이라 이 버그를 통과시켰다. 이번엔 실제 스폰 경로(`ConfigureEnemyScaling` → `SetEnemyPrefab`)로 돌려 동시 표시 프레임 4종, 방향 불일치 0마리를 확인했다.
- **크로마키 — 색으로는 못 가른다는 결론**: 갇힌 배경은 테두리 flood가 도달하지 못해 남는다. 씨앗을 넣고 다시 번지게 했더니 이번엔 **기사의 보라색 눈이 흰색으로 변했다**(캡처로 발견). 원인을 재 보니 **보라 글로우의 바깥 falloff가 배경색과 거리 5까지 겹친다** — 평탄화 시점에 이미 정보가 사라져 어떤 임계값을 잡아도 한쪽이 손해다. 결국 번짐 없이 순수 키색(거리 40 미만)만 지우는 쪽으로 정착했다.
- **공정 판단**: 앞으로의 아트는 **디자이너가 알파 채널을 살려 PNG로 내보내는 쪽이 맞다**. 크로마키 복원은 원본에 있던 정보를 추측으로 되살리는 작업이라, 이번처럼 캐릭터 색이 배경색과 겹치면 원리적으로 완전 복원이 불가능하다. 사용자에게 이 근거와 함께 전달할 요청 사항을 정리했다.
- **담당**: 개발

### 2026-08-10 (D-0) | 현대·미래 근접 몹 적용 — 4시대 몹 아트 완결

- **도구**: Claude Code (Opus 5) / Unity MCP (반입 · 실스폰 검증)
- **작업**: 디자이너가 **투명 PNG로 다시 준** 로봇(현대)·스토커(미래) 각 6프레임을 반입해 물렸다. 이로써 4시대 근접 몹이 전부 애니메이션된다.
- **프롬프트 원문**: "순서대로 현대몹 스프라이트, 미래몹 스프라이트야 각각 6장 이미지고 투명화작업까지 다 되어있어 좌우랑 애니메이션까지 적용해줘"
- **투명 PNG로 받은 효과**: 앞선 기사 작업에서 크로마키 복원이 원리적으로 불완전하다는 것을(보라 글로우와 배경색이 거리 5까지 겹침) 확인하고 요청했던 건이다. 이번엔 **키잉 코드가 아예 필요 없었고**, 크롭·축소만으로 끝나 한 번에 통과했다. 파일명에 `Walk_01~06`이 박혀 있어 프레임 순서 추측도 불필요했다.
- **설계 판단과 근거**: 앞 두 시대와 같은 규칙을 그대로 적용했다 — 6장 **공통 크롭**(로봇 583×501, 스토커 534×513), **알파 가중 평균** 축소, **세로 192px = 1유닛**. 세로 기준을 통일했기 때문에 시대가 바뀌어도 몹의 화면 크기가 튀지 않는다(로봇 1.17×1.00, 스토커 1.04×1.00 유닛).
- **검증 방법**: 실제 스폰 경로(`ConfigureEnemyScaling` → `SetEnemyPrefab`)로 두 시대를 돌려 각각 **서로 다른 프레임 5종 동시 표시 / 방향 불일치 0마리**. 미래 스토커의 보라 발톱 글로우가 캡처에서 유지되는 것도 확인.
- **누적 용량**: 근접 몹 4종 합계 1.8 MB(소스 PNG). 빌드 33.86 MB.
- **담당**: 개발(반입) / 디자인(로봇·스토커 각 6프레임, 투명 PNG)

### 2026-08-10 (D-0) | 아트 방향을 시대별 설정으로 분리 + 레벨업 최대 체력 성장

- **도구**: Claude Code (Opus 5) / Unity MCP (방향 실측 캡처 4회 · 레벨업 실측)
- **작업**: 좌우 반전을 시대별로 지정할 수 있게 만들고, 레벨업마다 최대 체력이 조금씩 오르게 했다.
- **프롬프트 원문**: "좌우 반대로 적용된듯하네 다시 수정해주고 내 레벨이 오를 수록 최대체력이 소폭 증가하게 해주고" → "아니아니 현대만 이미지가 왼쪽을 보고있고 나머지는 이미지 원본이 오른쪽을 보고있는게 맞아"
- **처음에 통째로 뒤집지 않은 이유**: 반전 신고를 받고 바로 상수를 뒤집는 대신 먼저 쟀다. 공룡을 좌우에서 걸어오게 해 캡처하니 **양쪽 다 플레이어를 향하고 있었다** — 즉 원시는 원래 맞았고, 통째로 뒤집었으면 멀쩡한 세 시대가 망가질 뻔했다. 그래서 코드에 방향을 박는 대신 `EraConfig.enemyArtFacesLeft`로 **세트별 지정**으로 바꿨고, 실제로 사용자 확인 결과 "현대만 왼쪽"이라 값 하나로 끝났다.
- **설계 판단과 근거**: 시대마다 아트 구도가 다르고 앞으로 원거리 몹·보스가 더 들어오는데, 방향이 통일된다는 보장이 없다. 코드 상수로 두면 세트가 늘 때마다 코드를 고쳐야 하지만, 설정값이면 인스펙터 체크 하나다. `_flipApplied` 플래그도 같이 넣었다 — `ArtFacesLeft`가 스폰 직후에 설정되는데 그 전에 `_facingLeft`가 우연히 맞아떨어져 있으면 갱신을 건너뛰어, 뒤집혀야 할 개체가 원본 그대로 남는다.
- **레벨 체력 성장**: 레벨당 최대 HP +4(기본 100). 증강 '균열 복구'(+20%)와 역할을 갈랐다 — 저쪽은 골라야 얻는 큰 한 방, 이건 고르지 않아도 쌓이는 바닥값이다. 크게 주면 증강을 뽑을 이유가 사라지므로 레벨 10에 +36(36%)으로 증강 한 장보다 조금 나은 선에서 멈췄다. `Health.IncreaseMaxHp`가 같은 양만큼 회복도 해줘서 레벨업이 곧 소량 회복이 된다.
- **검증 방법**: 레벨 1→11에서 최대 HP 100→**140**(10레벨 × 4), 다쳐 둔 현재 HP도 70→110으로 같이 상승(다단계 레벨업 경로 포함). 방향은 실제 스폰 경로로 원시(오른쪽 아트)와 현대(왼쪽 아트)를 대조 캡처 — 원시는 오른쪽 개체가 flipX=True로 왼쪽을 보고, 현대는 왼쪽 개체가 flipX=True로 오른쪽을 봐서 **정반대로 동작**하는 것을 확인.
- **검증 함정**: 중간에 `Object.Instantiate`로 직접 스폰해 확인했더니 `ArtFacesLeft`가 안 들어가 예전 값이 나왔다. 이 값은 `WaveManager.SpawnEnemy`에서만 주입되므로 **실제 스폰 경로로만** 검증이 성립한다.
- **담당**: 개발

### 2026-08-10 (D-0) | 원시 몹을 투명 PNG 랩터로 교체

- **도구**: Claude Code (Opus 5) / Unity MCP (반입 · 실스폰 검증)
- **작업**: 원시 시대 몹을 디자이너가 다시 준 **투명 PNG 랩터 6프레임**(`Raptor_Run_01~06`)으로 교체했다.
- **프롬프트 원문**: "원시시대 몹 투명화 제대로 해서 들고왔어 원본이미지는 오른쪽을 보고 있어 이걸로 교체해줘"
- **설계 판단과 근거**: **파일명을 `primal_dino_0~5`로 그대로 유지해 덮어썼다.** 새 이름으로 만들면 GUID가 새로 생겨 씬의 `enemyWalkFrames` 6칸을 다시 연결해야 하는데, 같은 경로에 덮어쓰면 GUID가 유지돼 배선이 그대로 산다(실제로 연결 6칸이 온전한 것을 확인). 덮어쓰기는 재임포트를 유발해 임포터 설정이 초기화될 수 있으므로 PPU·알파·압축을 다시 걸어 줬다.
- **처음 받은 판과의 차이**: 8/09에 받은 첫 공룡은 알파는 있었지만 투명부에 파란 RGB가 남아 있었고, 이번 랩터는 완전히 정리돼 있었다. 크롭·축소만으로 끝났다.
- **검증 방법**: 공통 경계 578×427 → 260×192, 6장 합계 420 KB, 스프라이트 1.35×1.00 유닛. 실제 스폰 경로로 **서로 다른 프레임 4종 동시 표시 / 방향 불일치 0마리**. 원본이 오른쪽을 보므로 `enemyArtFacesLeft = false`가 그대로 맞고, 캡처에서 왼쪽 개체가 원본 방향(오른쪽)으로 플레이어를 향해 달리는 것을 확인.
- **담당**: 개발(반입) / 디자인(랩터 6프레임, 투명 PNG)

### 2026-08-10 (D-0) | 모래시계·총알·보스탄 아트 적용 (원거리 몹은 제외 확정)

- **도구**: Claude Code (Opus 5) / Unity MCP (프리팹 주입 · 4방향 캡처)
- **작업**: 시대 전환 모래시계, 총잡이 총알, 분출구/보스 탄 세 오브젝트를 컬러 아트로 교체했다. **원거리 몹은 넣지 않기로 확정**됐다.
- **프롬프트 원문**: "원거리 몹은 안넣을거야 그렇게 알고있어 ... 이 모래시계 이미지를 보스를 잡고 다음스테이지로 갈때 먹는 중앙 물건을 이걸로 해주고 ... 이건 거너가 쏘는 총알로 ... 이건 포탑이 쏘는 총알로 바꿔줘 크기는 알아서 알맞게 조절해줘"
- **"포탑이 쏘는 총알"의 실체 확인**: 코드를 보니 `RiftVent`(분출구=포탑)가 **`BossProjectile`을 그대로 재사용**한다. 즉 프리팹 하나만 바꾸면 분출구와 보스 탄이 동시에 바뀐다. 화려한 금·보라 로켓이 보스/시간 테마와도 맞아 그대로 물렸다.
- **설계 판단과 근거**: 크기를 스케일이 아니라 **PPU로 잡았다** — 목표 월드 길이에서 PPU를 역산하면(`출력폭 / 목표길이`) 프리팹 스케일을 1로 유지할 수 있고, `BulletSizeMultiplier`처럼 스케일을 곱하는 증강과 충돌하지 않는다. 결과: 모래시계 0.70×1.21 / 총알 0.50×0.23 / 보스탄 0.65×0.24. **콜라이더도 새 그림에 맞춰 다시 잡았다** — 특히 보스탄은 왼쪽 40%가 배기 불꽃이라 판정에서 빼고 몸통만 잡아 오프셋 +0.11을 줬다(불꽃에 맞아 죽으면 억울하다). 세 프리팹 모두 `SpriteRenderer.color`를 흰색으로 되돌렸다 — 기존 틴트(금/미색/적색)를 그대로 두면 컬러 아트에 곱해져 색이 죽는다.
- **검증 방법**: 세 오브젝트를 한 화면에 띄워 캡처. 보스탄은 상하좌우 4방향으로 발사해 `transform.right` 회전이 진행 방향을 따르는 것과 배기 불꽃이 뒤쪽에 붙는 것을 확인. 총알·보스탄 모두 원본 아트가 오른쪽을 가리켜 회전 규약과 일치한다.
- **담당**: 개발(반입) / 디자인(오브젝트 3종)

### 2026-08-10 (D-0) | 모래시계에 부유 연출 (그림자 대비로 떠 있음을 만든다)

- **도구**: Claude Code (Opus 5) / Unity MCP (위상 다른 3개 동시 배치 실측 · 캡처)
- **작업**: 시대 전환 모래시계가 가만히 놓여만 있던 것을 위아래로 떠다니게 했다.
- **프롬프트 원문**: "모래시계가 그냥 멍하니 있는것 같아서 별로네 좀 둥둥 떠다니는 느낌을 주고 싶어"
- **설계 판단과 근거**: 상하 이동만으로는 "떠 있다"가 아니라 "흔들린다"로 읽힌다. **그림자를 같이 넣고 높이에 반비례하게 줄이고 옅게** 만들어야 부유가 성립한다 — 올라갈수록 그림자가 작아지고 흐려지는 대비가 실제 정보다. 여기에 기울기(±5도)를 상하와 **다른 주기**(1.55 vs 2.4)로 겹쳐 기계적 반복을 없앴고, 보라 글로우가 높이에 따라 숨 쉬듯 커졌다 작아진다.
- **판정은 띄우지 않았다**: 본체 스프라이트를 런타임 자식으로 옮기고 루트의 SpriteRenderer를 껐다. 루트를 직접 움직이면 콜라이더까지 같이 떠서 **주울 수 있는 자리가 시시각각 바뀐다** — 이 프로젝트에서 계속 지켜 온 "보이는 것 = 판정" 원칙과도 어긋난다.
- **검증 방법**: bobSpeed를 2.4 / 3.3 / 4.2로 어긋낸 3개를 한 화면에 놓고 캡처 — 본체 y가 +0.008 / −0.119 / −0.179로 갈리고, 그림자 폭·알파가 그에 반비례(0.45/0.26 ↔ 0.52/0.34)하는 것을 확인. 루트 y는 3.080으로 고정. 마침 테스트 세션이 길어져 **게임이 실제로 보스를 잡고 떨어뜨린 모래시계**가 같은 화면에 잡혀 실전 경로도 함께 확인됐다.
- **담당**: 개발

### 2026-08-10 (D-0) | 시대별 포탑·포탑탄 아트 + 포탑이 플레이어를 향해 고개를 돌린다

- **도구**: Claude Code (Opus 5) / Unity MCP (8기 동시 배치 실측 · 캡처)
- **작업**: 분출구(포탑) 기믹을 시대마다 다른 컬러 아트로 바꾸고, 쏘는 탄도 시대별로 갈랐다. 포탑이 플레이어 쪽을 바라본다.
- **프롬프트 원문**: "숫자 순서대로 원시, 중세, 현대, 미래 컨셉의 포탑과 총알이야 각 시대의 포탑기믹 이미지를 이것들로 적용시켜줘 그리고 플레이어 위치 기준으로 몬스터처럼 왼쪽 오른쪽 반전을 해서 대가리를 돌렸으면 좋겠고 원이미지의 포탑이 보는곳은 4개 전부 오른쪽으로 보고있어"
- **설계 판단과 근거**: 프리팹을 4개로 늘리지 않고 **`RiftVent.Spec`에 그림 두 장을 실어 보내는 방식**을 택했다. 분출구는 `EraHazardSpawner`와 `Boss`(VentDeploy) 두 곳에서 스폰되는데, 프리팹을 나누면 두 곳 모두 시대별 참조 4개씩을 들고 있어야 한다. Spec에 실으면 시대 설정 한 곳(`EraConfig.ventSprite` / `ventProjectileSprite`)만 채우면 된다.
- **틴트를 꺼야 했던 이유**: `ApplyVisual`이 매 프레임 `_spec.color`를 렌더러에 곱한다(예고/활성/소멸 알파 연출을 위해). 컬러 아트에 그대로 곱하면 금·보라가 죽으므로 `_hasArt`일 때만 흰색으로 바꿨다 — **알파 연출은 그대로 살렸다.**
- **반전은 flipX로만**: 분출구는 제자리 고정이라 이동 방향으로 판단할 수 없어 플레이어 x를 직접 본다. 스케일은 `ApplyVisual`이 `size / spriteWidth`로 매 프레임 덮어쓰므로 스케일 반전은 즉시 지워진다 — `flipX`가 유일하게 살아남는 방법이다. 플레이어가 포탑을 지나쳐 가면 고개가 따라 돌도록 Update에서도 갱신한다.
- **탄 크기**: 포탑은 `Spec.size`가 스케일을 정규화해 PPU가 결과에 안 남지만, 탄은 남는다. 시대별 탄을 0.55~0.60 월드 길이로 맞춰 어느 시대에서도 같은 크기로 읽히게 했다.
- **검증 방법**: 4시대 × 좌우 = 포탑 8기를 한 화면에 놓고 **방향 일치 8 / 불일치 0**, 각 시대 스프라이트가 제 것으로 붙은 것을 확인. 발사된 탄 46발을 세어 `ventshot_primitive/medieval/modern/future`가 11~12발씩 **섞이지 않고** 나가는 것도 확인.
- **담당**: 개발(반입·구현) / 디자인(포탑 4 + 탄 4)

### 2026-08-10 (D-0) | 4시대 보스 애니메이션 + 보스 판정을 그림에 맞춤

- **도구**: Claude Code (Opus 5) / Unity MCP (4시대 순차 스폰 실측 · 4기 정렬 캡처)
- **작업**: 흰 사각형이던 보스를 시대별 컬러 아트 애니메이션으로 바꿨다(원시/중세/현대 6프레임, 미래 8프레임). 보스 콜라이더를 그림 크기에 맞춰 늘렸다.
- **프롬프트 원문**: "이렇게 6프레임 스프라이트이미지가 각각 들어가있어 적용시켜줘" / "아 미래보스는 8프레임이야"
- **설계 판단과 근거**: 잡몹에 쓰던 `SpriteWalkAnimator`를 그대로 재사용했다 — `enemyWalkFrames`가 이미 가변 길이 배열이라 미래 8장도 코드 변경 없이 돈다. **PPU를 세로 256으로 잡아 스프라이트를 1유닛 높이로 만들었다**: 프리팹 스케일 3이 그대로 3유닛이 되므로 `Boss.Telegraph()`가 캐시한 `_baseScale` 펄스 계산을 건드리지 않는다.
- **보스에만 켠 것 — `facePlayerWhenIdle`**: 잡몹은 쉬지 않고 달려들어 이동 방향만으로 충분하지만, 보스는 패턴 시전 중 오래 서 있어 마지막 걷던 방향으로 등을 보였다(방향 일치 41/60). 멈췄을 때만 플레이어를 보게 하고 **이동 중에는 이동 방향이 이기게** 했다 — 돌진으로 플레이어를 지나칠 때 미끄러지듯 뒤돌아보지 않게 하려는 것이다. 잡몹 프리팹은 껐다(개체마다 매 프레임 Player 탐색 비용).
- **판정을 그림에 맞춘 이유**: 프리팹 콜라이더는 흰 사각형 시절의 3×3 고정인데 아트는 티렉스 4.88 · 전차 4.73 폭이라 **머리·꼬리·포신이 판정 밖**이었다. `PlayerWeapon.TargetRadius`가 콜라이더 bounds를 읽으므로, 8/9에 고쳤던 "칼날이 닿았는데 데미지가 안 들어감"이 보스 아트를 넣자마자 되살아날 자리였다. 스폰 시 스프라이트 크기의 x 0.85 / y 0.90으로 맞춘다(사각 판정이라 꼬리 위 빈 공간을 조금 덜어냄).
- **검증 방법**: 4시대를 순차 스폰해 실측 — 프레임 6/6/6/**8**종 전부 순환, 렌더러 색 전부 흰색(틴트 미적용), 방향 일치 46/54 · 70/71 · 71/71 · 70/71, 판정/그림 비율 4시대 모두 85%(3.00 고정 → 원시 4.14 · 현대 4.02). `Object.Instantiate`가 아니라 `ConfigureBoss` → `ForceSpawnBoss` 실제 경로로 돌렸다.
- **오진 하나**: 첫 캡처에서 티렉스가 주황으로 보여 틴트가 아트를 죽인 줄 알았으나, `sr.color`를 재 보니 순백이었다 — Scene View가 입 안(갈색 잇몸)까지 확대돼 있었을 뿐이다. 색을 눈으로 판정하려 한 것이 잘못이었다.
- **담당**: 개발(반입·구현) / 디자인(보스 4종 26프레임)

### 2026-08-10 (D-0) | 플레이어 호버 애니메이션 + 거너폼 총(발사 반동 연동)

- **도구**: Claude Code (Opus 5) / Unity MCP (좌·우·위 조준 실측 + 캡처 5장)
- **작업**: 시안 흰 사각형이던 플레이어를 호버 로봇 6프레임으로 바꾸고, 거너폼에만 붙는 총을 만들었다. 총알이 나갈 때마다 8프레임 발사 애니메이션이 한 번 재생된다.
- **프롬프트 원문**: "이게 플레이어 캐릭터야 방향키에 따라 좌우 맞춰서 적용시켜주고 ... 이게 거너폼 무기야 캐릭터랑 같이 다니고 총알이 발사될때마다 애니메이션이 그거에 맞게 따라서 재생될수있게 해줘"
- **원본을 먼저 재고 정한 것 둘**: (1) 플레이어 6장은 x가 353~923으로 **완전히 고정**이고 y만 변한다 → 제자리 호버. (2) 총 8장은 y가 고정이고 x가 1\~3번 → 4\~5번(250px 왼쪽) → 6\~8번 복귀 → **반동을 위치 이동으로 그린 것**. 둘 다 공통 크롭으로 반입해 그 움직임을 코드 없이 살렸다. 프레임마다 잘랐으면 호버도 반동도 통째로 사라졌을 것이다.
- **피벗을 몸통 중심(0.630)에 둔 이유**: 프레임 616px 중 몸통은 348px이고 나머지는 아래로 뻗는 추진 화염이다. 가운데를 피벗으로 잡으면 판정(1×1 콜라이더)이 화염 쪽으로 내려앉는다. 행별 불투명 픽셀 수로 몸통/화염을 갈라 몸통 중심을 6장 평균(0.630)으로 냈고, 몸통 348px = 1유닛으로 스케일해 **콜라이더를 한 픽셀도 안 건드렸다**(난이도 불변).
- **총을 조준 방향으로 돌린 판단**: 요청은 "같이 다니고 발사 때 재생"까지였지만, 자동 조준이라 적이 어느 방향에든 있는데 총이 반대쪽을 보며 반동만 치면 고장 난 것으로 읽힌다. 회전 + `flipY`(왼쪽 조준 시)로 세운다. **적이 없을 때는 방향키가 보는 쪽으로 총구가 돌아간다** — 플레이어 그림이 정면 대칭이라 flipX가 눈에 안 보이므로, 방향키 입력이 화면에 드러나는 통로가 이것뿐이다.
- **`SpriteWalkAnimator`를 재사용하지 않은 이유**: 적은 이동량(dx)으로 방향을 정하는데 플레이어는 **입력**으로 정해야 한다(벽에 붙어 밀면 이동량이 0이라 방향이 굳는다). 적·보스 전부가 매달린 컴포넌트를 마감 직전에 건드리지 않으려는 목적도 있다.
- **검증 방법**: 실제 스폰/발사 경로로 오른쪽 0°·왼쪽 180°(flipY)·위 85°를 실측하고 캡처 5장. 발사 프레임 8/8, 호버 프레임 6/6 순환 확인. 칼잡이로 바꾸면 총 렌더러가 꺼지는 것(활성 True→False)도 확인.
- **오진 하나 + 못 한 검증 하나**: 첫 캡처에서 총이 안 보여 정렬 순서를 의심했으나, 실측하니 **플레이어가 죽어 있었다**(HP 0/100 — 앞선 프로브를 오래 방치하는 동안 사망). 사망 시 총을 숨기는 건 의도된 동작이라 코드는 정상이었다. 그리고 **방향키 반전은 합성 입력으로 검증하지 못했다** — 에디터 Game 뷰에 포커스가 없어 `InputSystem.QueueStateEvent`가 게임 루프에 도달하지 않는다(`isPressed`는 True인데 `PlayerMove.LastInput`은 (0,0)). 사람이 브라우저에서 확인해야 한다.
- **담당**: 개발(반입·구현) / 디자인(플레이어 6 + 총 8프레임)

### 2026-08-10 (D-0) | 중세 보스 좌우 반전 수정 + 미래 선회 레이저 감속

- **도구**: Claude Code (Opus 5) / Unity MCP (프리팹 직렬화 값 실측 + 좌우 배치 캡처)
- **작업**: 중세 보스가 반대쪽을 보던 것을 고치고, 미래 보스의 선회 레이저 회전 속도를 2.4배 낮췄다.
- **프롬프트 원문**: "중세시대 보스 좌우 방향이 반대야 수정해줘 그리고 미래시대 보스 레이저패턴이 너무 빨라 좀 느리게 할 필요가 있어"
- **중세 오판의 원인**: 보스 아트를 넣을 때 기사 그림을 "정면 대칭이라 방향 무관"으로 판단해 `bossArtFacesLeft=false`로 뒀다. 실제로는 검이 화면 왼쪽, 망토가 오른쪽으로 흐르는 **왼쪽 지향** 구도라 반전이 정확히 거꾸로 걸렸다. 대칭처럼 보이는 그림도 소품(검·망토)이 방향을 만든다 — 실루엣만 보고 판단한 게 잘못이었다.
- **레이저 감속**: 코드 기본값이 아니라 **프리팹에 직렬화된 값**을 먼저 읽어 원인을 특정했다(코드 쪽 미래 세트에는 OrbitSweep이 아예 없다). 8단계 × 25° / 0.13초 = **192°/초**에 예고가 0.117초뿐이라 나타나는 즉시 발사되는 셈이었다. 10단계 × 20° / 0.25초 = **80°/초**로 낮췄다.
- **예고/발사 배수를 함께 바꾼 이유**: 둘 다 `orbitStepTime` 비례라 단계 시간만 늘리면 발사 창까지 같이 늘어 오히려 더 아프다. 예고를 한 단계보다 **길게**(0.9 → 1.6배 = 0.4초) 잡아 다음 한두 단계 막대가 미리 깔려 회전 방향이 부채꼴로 보이게 하고, 발사 창은 한 단계보다 **짧게**(0.8 → 0.7배 = 0.175초) 줄여 스쳐도 여러 번 맞지 않게 했다.
- **검증 방법**: 중세 보스를 실제 스폰 경로로 띄워 방향 일치 **2145/2145**, 플레이어 좌우에 기사를 한 기씩 놓은 캡처에서 검이 안쪽·망토가 바깥쪽으로 향하는 것 확인. 미래는 위성 선회가 나올 때까지 기다렸다 패턴 전체 **5.20초**(예고 0.7 + 훑기 2.75 + 후딜 1.6) 측정, 막대 21개 생성 / 동시 최대 9개.
- **담당**: 개발

### 2026-08-10 (D-0) | EXP 구슬을 컬러 아트로 교체

- **도구**: Claude Code (Opus 5) / Unity MCP (알파 히스토그램 + 실제 드롭 경로 캡처)
- **작업**: 시안으로 물들인 흰 사각형이던 경험치 오브를 결정 구체 아트로 바꿨다.
- **프롬프트 원문**: "EXP구슬을 이걸로 바꿔줘"
- **본체와 후광을 갈라 크기를 잡았다**: 알파 128 경계(800px)를 결정 구슬 지름으로 착각했는데, 임계값을 8까지 낮춰도 804px로 4px밖에 안 늘어나 **그 800px이 이미 반짝임까지 포함한 전체**임이 드러났다. 행별 불투명 픽셀 수가 300을 넘는 행만 본체로 세어 실제 구슬은 607px(전체의 0.755)로 측정했고, 그 본체가 0.50유닛이 되게 PPU를 잡았다(기존 흰 사각형 0.40보다 조금 크게).
- **스케일 대신 PPU**: 프리팹이 스케일 0.4로 크기를 내고 있었는데 스케일을 1로 되돌리고 PPU로 크기를 정했다. 스케일에 크기를 실으면 나중에 스케일을 곱하는 무언가와 섞였을 때 원인을 못 찾는다. 스케일이 0.4→1로 바뀌므로 `CircleCollider2D.radius`를 0.5→0.2로 줄여 **줍는 지름 0.40을 그대로 유지**했다(게임 감각 불변).
- **틴트 제거**: 프리팹이 시안(0.435, 0.847, 0.878)을 곱하고 있었다. 컬러 아트에 곱하면 보라 링과 금색 반짝임이 죽으므로 흰색으로 바꿨다 — 적·보스·포탑과 같은 처리다.
- **검증 방법**: 웨이브 스포너가 낸 실제 적 6마리를 플레이어에게서 떨어뜨려 죽이고 **실제 드롭 경로**로 나온 오브를 확인 — 6/6이 `exp_orb`, 색 순백, 스케일 1, 화면 0.66, 콜라이더 지름 0.40. 갈색 바닥 위 캡처에서 보라 링·시안 코어가 배경과 갈리는 것도 확인.
- **용량**: 1254×1254 원본을 128×128로 줄여 31 KB. 오브는 화면에 수십 개가 동시에 깔리는 물건이라 텍스처를 키울 이유가 없다(실제 게임 줌에서 약 24px로 그려진다).
- **담당**: 개발(반입·구현) / 디자인(결정 구체 1장)

### 2026-08-10 (D-0) | EXP 구슬 크기 축소 (0.50 → 0.36유닛)

- **도구**: Claude Code (Opus 5) / Unity MCP (실제 게임 줌으로 재캡처)
- **작업**: 바로 앞 항목에서 0.50유닛으로 넣은 EXP 구슬이 크다는 지적을 받아 0.36유닛으로 줄였다.
- **프롬프트 원문**: "구슬 크기가 좀 큰거 같아 좀 줄여줘"
- **왜 컸는지**: 아트를 넣을 때 "기존 흰 사각형 0.40보다 조금 크게"를 근거로 0.50을 잡았는데, 그 0.40은 **사각형의 한 변**이라 대각선까지 세면 오히려 새 구슬이 더 컸다. 게다가 새 아트는 바깥 후광까지 붙어 프레임이 0.66이라 체감은 그보다 더 컸다.
- **직전 캡처를 확대 줌(ortho 3.0)으로 찍은 것도 오판을 도왔다**: 그 화면에서는 적당해 보였지만 실제 게임 줌은 **ortho 5**다. 이번에는 카메라 값을 건드리지 않고 게임이 쓰는 줌 그대로 찍었다 — 크기 판단은 실제 줌에서만 의미가 있다.
- **바꾼 것은 PPU 하나뿐**: 프리팹 스케일(1)도 콜라이더(줍는 지름 0.40)도 그대로다. 크기를 PPU에 실어 둔 덕에 한 값만 고치면 끝났다.
- **검증 방법**: 실제 적 8마리를 처치해 드롭시킨 뒤 게임 줌 ortho 5로 캡처 — 프레임 0.66 → **0.48**, 결정 본체 0.50 → **0.36**, 화면상 35px → **25px**. 갈색 바닥에서 보라 링·시안 코어가 여전히 갈리는 것도 같은 캡처에서 확인.
- **담당**: 개발

### 2026-08-10 (D-0) | 칼잡이: 참격 이펙트를 버리고 홀 그림 자체를 히트박스로

- **도구**: Claude Code (Opus 5) / Unity MCP (주축 각도·회전축 실측, 위치별 피해 실측)
- **작업**: 근접무기 10프레임을 반입하고, 호(弧) 스프라이트 판정을 없앤 뒤 **휘두르는 홀 그림이 지나간 자리**가 곧 판정이 되게 바꿨다.
- **프롬프트 원문**: "근접무기 10프레임 짜리 이미지가 들어가 있어 이걸 공격마다 애니메이션이 재생되게 적용해줘" → "근접 이팩트를 버리고 지금 추가한 이미지를 휘둘러서 저 이미지에 히트박스를 만들고 이미지에 맞았을때 적에게 데미지가 들어가도록 해줘"
- **그림에서 궤적을 읽어냈다**: 10장의 주축을 2차 모멘트로 재니 0° → +27°(들어올림) → −34°(내려침) → 0°(복귀)였고, 회전축을 최소제곱으로 풀었더니 자루에서 47% 지점(자루 중간)이었다. **무게중심 예측 오차 5px** — 순수 회전이 맞다는 뜻이다. 그래서 공통 크롭 + 그 회전축을 스프라이트 피벗으로 반입했고, 프레임별 각도표를 그대로 코드에 넣어 판정이 같은 각도를 쓰게 했다.
- **자세의 주인을 뒤집었다**: 그림(BladeVisual)이 자세를 정하고 판정이 따라가면 둘이 어긋날 수 있다. 요청 자체가 "그림이 히트박스"이므로 **BladeWeapon이 자세와 판정을 같은 함수 안에서 함께 내고**, 그림은 프레임 번호·회전·반전을 받아 LateUpdate에 그리기만 한다. 어긋남을 만들 수 없는 구조다.
- **판정 모양**: 홀은 가늘고 긴 막대라 원/부채꼴이 아니라 **선분(자루 끝 ↔ 머리 끝) + 굵기 0.14**의 막대 판정이다. 적의 몸 반지름을 더해 보스(스케일 3)도 몸통이 걸리면 맞는다. 한 번 휘두르는 동안 각 적은 **한 번만** 맞는다(스윙별 피격 집합).
- **증강을 죽이지 않으려고 다시 맞춘 것**: 사거리 증강은 홀의 **크기 배율**로(그림과 판정이 같이 커진다), 호 확장 증강은 그려진 각도에 **배율**을 걸어 더 크게 휘두르는 것으로 바꿨다. 후방 참격은 플레이어 기준 180° 반대편에 홀을 하나 더 그리고 같은 막대 판정을 준다.
- **검증 방법**: 적 5마리를 고정 배치하고 3초(스윙 5회) 실측 — 앞 0.9 **130**, 앞 1.4 **130**(26×5, 중복 타격 없음), 앞 2.2 **0**, 옆 1.2 **0**, 뒤 1.0 **0**. 캡처에서 참격 호가 사라지고 홀만 남은 것도 확인.
- **의도적으로 남긴 부작용**: 사거리가 **2.4 → 1.77**(홀 끝 1.13 + 굵기 0.14 + 적 반지름 0.5), 타격 각도가 130° → 61°로 줄었다. 그림이 판정이므로 필연이다. 다만 공격 간격 0.55초에 스윙이 0.5초라 **판정이 91% 시간 살아 있어** 체감 손실은 수치보다 작다. 더 늘리려면 홀을 크게 그리는 수밖에 없다(사용자 판단 대기).
- **합치지 않은 것**: GunVisual과 구조가 겹치지만 통합하지 않았다. 검증이 끝난 거너 경로를 마감 직전에 건드리지 않는 편이 안전하고, 프로젝트 규칙도 요청 없는 리팩터링을 금한다.
- **담당**: 개발(반입·구현) / 디자인(홀 10프레임)

### 2026-08-10 (D-0) | 시대별 BGM 4곡 반입 + 전환 시 크로스페이드

- **도구**: Claude Code (Opus 5) / Unity MCP (임포트 설정·플레이 모드 4시대 실측)
- **작업**: 디자인 담당이 준 시대별 BGM 4곡을 `Assets/Resources/BGM/`에 반입하고, `BgmManager.cs`(EraManager 오브젝트에 부착)가 `EraManager.OnEraChanged`(이번에 신설)를 구독해 곡을 1초 크로스페이드로 갈아끼우게 했다. 게임에 오디오가 들어간 것은 이번이 처음이다.
- **프롬프트 원문**: "C:\Users\USER\Desktop\image\sound 여기에 시대별 이름으로 폴더를 만들어뒀고 bgm을 넣어뒀어 각 시대에 bgm을 추가해줘"
- **용량이 결정을 지배했다(WebGL)**: 원본 MP3 6.4 MB는 33 MB 빌드에 그대로 얹기엔 컸다. **모노 강제 + Vorbis 품질 0.4**로 재인코딩해 **2.0 MB**(-69%)로 줄였다. 배경음악은 2D 재생이라 스테레오를 버려도 손실이 체감되지 않는다. 로드 타입은 `CompressedInMemory` — WebGL은 Streaming을 지원하지 않고, `DecompressOnLoad`는 4곡을 PCM으로 풀면 수십 MB가 된다.
- **4곡을 인스펙터 참조로 안 물린 이유**: 참조로 물리면 씬 로드 시 4곡이 전부 메모리에 상주한다. 대신 `Resources.Load`로 **그 시대에 들어갈 때만 로드**하고, 크로스페이드가 끝나면 이전 곡을 `Resources.UnloadAsset`으로 즉시 버린다. 저사양 노트북 브라우저가 타깃이라 상시 상주 오디오를 4개 둘 이유가 없다. 대가로 파일명 규약(`bgm_primitive`/`_medieval`/`_modern`/`_future`)이 Era enum 순서에 묶인다.
- **페이드는 `unscaledDeltaTime`으로 돈다**: 증강 카드가 뜨면 `timeScale=0`이라 스케일 시간으로 페이드를 돌리면 음악이 그 자리에 얼어붙는다. 같은 이유로 `AudioSource.ignoreListenerPause = true`.
- **검증 방법**: 플레이 모드에서 4시대를 연속 전환하며 AudioSource 2개의 상태를 매 시대 실측 — 원시 `bgm_primitive` 재생/볼륨 0.50 → 중세·현대·미래 각각 해당 곡으로 교체되고 **반대편 소스는 매번 clip=null·volume 0**으로 회수됐다(누수·중복 재생 없음). 콘솔 에러 0건.
- **담당**: 개발(반입·구현) / 디자인(BGM 4곡)

### 2026-08-10 (D-0) | 효과음 16종을 코드로 합성 + 타이틀 화면 BGM 차단

- **도구**: Claude Code (Opus 5) / Unity MCP (플레이 모드에서 파형 실측·이벤트별 발생 확인)
- **작업**: `SfxManager.cs` 신설 — **오디오 파일 없이 파형을 코드로 합성**해 효과음 16종을 만들고 게임 전체에 연결했다. 동시에 타이틀·직업 선택 화면에서는 BGM이 나지 않게 하고(판 시작 시점에 재생), 결과 화면에서는 1.2초 페이드아웃으로 걷어낸다.
- **프롬프트 원문**: "메인 화면에서는 bgm 안나게 해줘야지 그리고 버튼 누르는 사운드 만들어서 넣어주고 내가 피격당했을때 사운드, 총쏘는 사운드, 보스 생성 사운드, 적 피격했을때 사운드 exp구슬 먹었을때 사운드등을 만들어서 넣어줘 더 필요한 사운드들이 있다고 생각이들면 너가 알아서 판단하고 사운드 제작후 적용시켜줘"
- **왜 합성인가(WebGL)**: 효과음 팩을 받으면 16종에 수 MB가 붙고 라이선스 표기까지 따라온다. 사인/사각/톱니/노이즈 + 포락선으로 구우면 **빌드 용량 증가 0바이트**, 라이선스도 우리 것이다. 대가는 실사 질감 포기 — 레트로 아케이드 톤이라 오히려 맞다. 22050Hz 모노로 굽고 **RAM 765 KB**를 쓴다(다운로드가 아니라 메모리라 WebGL에서 문제되는 축이 아니다).
- **진폭을 손으로 안 맞추고 정규화했다**: 오실레이터를 몇 개 겹치든 클립마다 최고점을 목표값으로 정규화한다. 소리 크기 설계가 `targetPeak` 한 값으로 끝난다 — **피격음 0.95 / 총 0.38 / 적 피격 0.40 / 경험치 0.35**로, 자주 나는 소리일수록 낮췄고 "내가 맞았다"는 대역(저역 사각파)까지 겹치지 않게 띄웠다.
- **요청 6종 + 판단으로 10종 추가**: 적 처치·레벨업·회복·모래시계 획득·시대 전환·보스 처치·근접 휘두르기·보스 피격(잡몹과 다른 낮은 음)·게임오버·클리어. 보스 12패턴 예고음은 **넣지 않았다** — 마감 직전에 검증 끝난 `Boss.cs` 패턴 코드를 12군데 여는 위험이 이득보다 크다.
- **호출부를 최소로 열었다**: 버튼 12개·플레이어 피격/사망·레벨업·시대 전환·클리어는 `SfxManager`가 씬 이벤트를 직접 구독해 처리한다(게임플레이 스크립트 수정 0줄). 나머지 8곳만 한 줄씩 넣었다. 적 피격은 무기별이 아니라 `Health.OnHit`에 걸어 총알·홀·장판 어디에 맞아도 누락이 없다.
- **저사양 대비**: 소리별 최소 간격(적 피격 45ms 등)으로 한 프레임에 20마리가 맞아도 한 번만 난다. 보이스 10개 풀 라운드로빈, 반복음은 피치를 ±6~10% 흔들어 기계음이 되지 않게 했다.
- **검증 방법**: 플레이 모드에서 16클립 파형을 전부 실측 — 무음 0건, 길이·피크가 설계값과 일치, **마지막 샘플이 전부 0.0000**(재생 끝 클릭 없음). 이벤트별로 보이스를 전부 끈 뒤 하나씩 일으켜 확인: 버튼(12개 전부 훅)·플레이어 피격·적 피격·적 처치·EXP 흡수·보스 등장·보스 피격·레벨업·사망(피격음+게임오버음 2보이스) 모두 발생. 타이틀에서 BGM 0, `StartRun()` 직후 `bgm_primitive` 0.50, 사망 후 `timeScale=0`에서도 페이드아웃이 끝까지 돌아 회수됨. 씬 재시작 후 재훅도 확인. 콘솔 에러 0건.
- **오진했던 것**: 중간에 "사망음이 안 난다"고 판단했는데, MCP 호출 사이에 게임이 계속 돌아 **이미 죽어 있던 플레이어**를 다시 때린 것이었다(`IsDead`면 `TakeDamage`가 즉시 반환). `timeScale=0`으로 얼린 뒤 다시 재니 정상 2보이스였다. 계측 대상이 계속 움직이는 상태에서 단발 스냅샷으로 판단한 것이 원인.
- **담당**: 개발

### 2026-08-10 (D-0) | 메인 화면 배경 아트 반입 (타이틀 + 직업 선택)

- **도구**: Claude Code (Opus 5) / Unity MCP (SerializedObject 경로 아닌 런타임 오브젝트 조작 + 플레이 모드 캡처)
- **작업**: 디자인 담당이 준 시간관리국 홀 아트를 타이틀 화면과 직업 선택 화면 배경으로 넣었다. `HUD_Canvas/TitlePanel`과 `ClassPanel` 각각에 `BackgroundArt`(sibling 0)를 만들어 기존 Overlay 아래에 깔았다.
- **프롬프트 원문**: "메인 화면 ui 뒷 배경을 이걸로 해줘"
- **용량(WebGL)**: 원본 1672×941 PNG 1.8 MB. **1280×720으로 줄여** 반입했다 — 4의 배수라 DXT1 압축이 걸려 **450 KB**가 된다. 원본 비율이 정확히 16:9(1.777)라 축소만으로 왜곡이 없다. 1280×713 같은 비(非)4배수로 들어가면 압축이 안 걸려 RGBA32 3.6 MB가 된다(그래서 임포트 전에 리사이즈했다). 알파는 버렸다(불투명 배경 + DXT1 조건).
- **늘리지 않고 덮었다**: `AspectRatioFitter(EnvelopeParent)`로 화면 비율이 16:9가 아니어도(테스트 창은 2.03) 그림이 늘어나지 않고 넘치는 쪽이 잘린다. 전체 스트레치는 원형 바닥이 타원으로 찌그러진다.
- **Overlay는 지우지 않고 낮췄다**: 두 화면의 어두운 판(알파 0.96 / 0.90)은 UI 가독성용이지 배경을 가리려던 게 아니라, **타이틀 0.32 / 직업 선택 0.62**로 낮춰 그림이 보이면서 글자가 읽히는 지점을 잡았다. 직업 선택이 더 어두운 이유는 카드가 배경 장식과 겹쳐 경계가 흐려지기 때문.
- **캡처로 발견해 고친 것**: 첫 결과에서 "요원 선택" 제목이 하필 배경 모래시계의 발광 위에 앉아 읽히지 않았다. 오버레이만 더 어둡게 하면 그림이 죽어서, 직업 카드와 **같은 스프라이트(`ui_panel_notch`)·같은 색**의 명패를 글자 뒤에 깔았다 — 새 아트 없이 기존 디자인 언어로 해결했다.
- **검증 방법**: 플레이 모드에서 두 화면을 실제 캡처해 확인(추정 아님). BackgroundArt 실측 크기 2050×1153(부모 2050×1011을 세로로 덮고 12% 잘림), sibling 0, raycastTarget=off(버튼 클릭을 먹지 않는다). 콘솔 에러 0건.
- **담당**: 개발(반입·배치) / 디자인(배경 아트 1장)

### 2026-08-10 (D-0) | 팀 소개 문서 작성 (제출물 5번) — 로그를 역할 기술서로 환원

- **도구**: Claude Code (Opus 5)
- **작업**: `docs/TEAM_ROLES.md` 신규. 디자인 담당이 작성한 역할 기술 원문을 그대로 싣고, 개발 담당 항목은 이 로그 78건에서 역산해 10개 영역으로 정리했다. 비어 있던 "팀원별 담당" 표도 채웠다.
- **프롬프트 원문**: "팀 소개 문서 팀원별 역할·담당 영역 정리 ... 내 역할 담당을 작업한 내용을 보고 정리해서 적어주고 [디자이너 역할 원문] 이게 디자이너의 역할 및 담당영역 내용이야 이거랑 합쳐서 마크다운 문서로 만들어줘"
- **설계 판단과 근거**: 개발 항목을 디자인 원문과 **같은 형식(소제목 + 서술 + 최종 역할)** 으로 맞춰 한쪽만 튀지 않게 했다. 기능 나열 대신 판단 근거가 남은 항목을 우선 올렸다 — AI 디렉터의 "개입은 항상 배너와 함께", 랭킹의 same-origin CORS 회피, 효과음 코드 합성(용량 0바이트)처럼 **WebGL 제약에서 나온 결정**이 심사에서 설명력이 크다고 봤다. AI 오진을 사람이 잡은 사례도 뺐다가 다시 넣었다(검증 책임 주체가 사람이라는 근거).
- **검증 방법**: 문서 내 수치는 추정하지 않고 실측했다 — 스크립트 55종·10,914줄(`grep -c`), 증강 23종(AugmentType enum + .asset 23개), 보스 12패턴·기믹 4종은 README와 대조.
- **AI 산출물 vs 사용자 개입**: 디자인 역할 원문은 사용자 제공(그대로 인용). 개발 역할 정리·구성·수치 실측은 AI. 팀원 이름은 `(이름)` 자리표시자로 두어 사용자가 채운다.
- **담당**: 개발(문서 작성) / 디자인(자기 역할 원문 제공)

### 2026-08-10 (D-0) | 장판 아트 2종 교체 + 근접무기를 몸에서 떼어냄

- **도구**: Claude Code (Opus 5) / Unity MCP (알파 픽셀 실측, 플레이 모드 캡처 5회)
- **작업**: 코드로 겹쳐 그리던 감속 지대(채움+테두리 4장+소용돌이)와 매지션 지대(채움+링+소용돌이)를 걷어내고 디자인 담당의 장판 아트 한 장씩으로 교체했다. 칼잡이 홀을 쥐는 지점을 몸 밖으로 뺐다.
- **프롬프트 원문**: "이게 맵 기믹이나 보스 기믹에 있는 슬로우 장판이고 이게 메지션폼때 사용하는 공격장판이야 현재 있는 이팩트들 지우고 이걸로 교체 적용해줘 그리고 근접무기 위치가 몸에 너무 겹쳐져있어 손에 잡고 휘두르는 듯한 연출을 위한거니까 좀 떨어뜨려서 휘두르게 해줘"
- **감속 지대 판정을 사각 → 원으로 바꿨다**: 예전 판정이 사각이었던 이유는 내장 Square로 그려서 보이는 사각형과 판정이 일치했기 때문이다. 원형 아트를 받은 이상 원을 그려 놓고 사각으로 맞히는 쪽이 더 나쁜 거짓말이라 `SpeedMultiplierAt`을 제곱거리 비교로 바꿨다. **부작용: 영향 면적이 옛 사각 대비 78.5%로 줄어든다**(내접원).
- **매지션 장판은 원근을 포기하고 세로를 늘렸다**: 원본이 원근으로 눌린 타원(가로 53.5% / 세로 37.5% — 픽셀 실측)이라 그대로 쓰면 세로 사거리가 실제의 절반처럼 보인다. 이 프로젝트가 계속 지켜 온 "보이는 것이 곧 판정"을 택해 세로 2.14배로 늘려 정원으로 만들었다(스케일 0.63 × 1.35). 캡처로 확인했을 때 늘어난 티가 거의 안 났다.
- **여백을 실측해서 스케일을 냈다**: 두 아트 모두 프레임에 큰 여백이 있어(그림이 68% / 53.5%) 스프라이트 크기를 그대로 쓰면 보이는 원이 판정보다 32~47% 작다. 알파 24 기준으로 그려진 범위를 재서 `FxSprites`에 상수로 박고 그 값으로 나눈다.
- **회전축을 위치가 아니라 피벗으로 잡았다**: 매지션 장판은 그림 중심이 프레임 중심보다 2.2% 아래다. `localPosition`으로 보정하면 장판이 회전할 때 그 오프셋만큼 원이 흔들린다(피벗이 곧 회전축이므로). 임포터에서 커스텀 피벗 (0.4995, 0.478)로 잡아 해결했다.
- **오래된 직렬화 값에 막혔다**: 코드 기본값을 0.9로 올려도 지대가 흐렸는데, RiftZone **프리팹에 옛 maxAlpha 0.42와 보라색 틴트가 직렬화**돼 코드 기본값을 덮고 있었다. 캡처로 알아채고 `SerializedObject`로 0.9·흰색으로 주입했다(아트에 색을 곱하면 그림이 그 색조로 덮인다 — 적·보스 컬러 아트에서 이미 겪은 문제).
- **홀 위치**: 플레이어 그림이 1.63×1.77유닛(반지름 약 0.8)인데 쥐는 지점이 0.45라 손이 몸 한가운데 있었고, 자루가 0.61 더 뒤로 뻗어 홀이 몸을 관통했다. **0.45 → 1.05**로 뺐다. 부수 효과로 유효 사거리가 1.77 → 2.37이 되는데, 이는 홀 도입 전 참격 시절의 2.4와 거의 같다 — 그때 "의도적으로 남긴 부작용"으로 적어 둔 사거리 손실이 이번에 되돌아온 셈이다.
- **검증 방법**: 플레이 모드 캡처로 세 가지를 눈으로 확인(추정 아님) — 감속 지대가 지름 6으로 그려지고 적이 그 안에 들어감, 매지션 장판이 판정 지름 5.20과 같은 정원으로 플레이어 중심에 붙음, 홀이 몸 밖 오른쪽에 들려 있음. 텍스처는 DXT5 256+384 = **641 KB**(실측 후 isReadable를 꺼 CPU 사본 제거). 콘솔 에러 0건.
- **중간에 헛다리**: 첫 캡처에서 감속 지대가 안 보여 스프라이트 로드 실패를 의심했는데, `timeScale=0`으로 얼려 놓아 페이드인이 진행되지 않아 알파가 0이었을 뿐이다(Update는 돌지만 deltaTime이 0).
- **담당**: 개발(반입·구현) / 디자인(장판 아트 2장)

### 2026-08-10 (D-0) | AI 활용 기술서 통합본 작성 (제출물 ④)

- **도구**: Claude Code (Opus 5)
- **작업**: 디자인 담당의 「시간관리국 AI 활용 기술서 v2.0」(아트·영상·크로마키 중심)과 개발 쪽 `AI_LOG.md` 86개 세션을 합쳐 `docs/시간관리국_AI_활용_기술서.md`(v3.0) 작성. 대회 요건 3가지(구조 설명 / 주요 프롬프트·지시 / 외부 에셋·오픈소스 출처)에 대응표를 붙였다.
- **프롬프트 원문**: "이게 디자이너가 ai 활용한 기술문서인데 읽어보고 지금 내 ai 활용했던 내용쭉 훑고 붙힐 내용 다 붙혀서 그거랑 같이 디자이너 ai 활용문서랑 내 ai 활용 문서 합쳐서 마크다운 파일로 만들어줘 참고로 bgm은 suno pro 결재해서 다 만들었어" / "AI 활용에 대한 기술적 설명(예: 구조 설명, AI 대상 주요 프롬프트 및 지시 사항) 외부 에셋 / 오픈소스 출처 이게 ai 활용문서 기준이야"
- **설계 판단과 근거**: 두 문서를 앞뒤로 이어 붙이지 않고 **공통 원칙 → 디자인 → 개발 → 통합 지점** 구조로 재편했다. 2인 팀에서 실제로 설명 가치가 있는 것은 각자의 작업이 아니라 **아트가 게임이 되는 접점**(그림에 히트박스를 맞춘 사례, 용량 예산, 프레임 규약)이라 이를 05장으로 독립시켰다. 개발 파트의 주요 프롬프트는 나열 대신 **4가지 패턴**(사양 못박기 / 하지 말 것 명시 / 검증 절차 포함 / 감각 지적 후 원인 규명)으로 분류했다 — 요건이 "프롬프트 나열"이 아니라 "지시 사항의 기술적 설명"이기 때문이다.
- **가장 강한 근거로 쓴 것**: `CLAUDE.md` 자체를 "가장 중요한 프롬프트"로 제시했다. 이 파일의 각 줄이 대부분 사고 이후 추가된 재발 방지 규칙(8분 세션 차단 → 외부 편집 금지 / 폰트 참조 11개 전멸 → Update Atlas Texture 강제)이라, AI 활용의 성숙 과정을 그대로 보여준다.
- **출처 항목 갱신**: BGM 4곡의 "제작 방식 미확인"을 **Suno (Pro 구독)** 으로 확정했다. 비어 있던 `AI 생성 에셋` 표도 5행으로 채웠고, 코드 합성 효과음·런타임 텍스처는 "생성형 AI 산출물"이 아니라 **AI가 쓴 코드가 실행 시 만드는 데이터**로 구분해 적었다.
- **남은 확인 사항**: 몹·보스·플레이어·무기·장판·배경 아트의 생성 도구를 기술서에는 ImageGen/Seedance로 적었으나, 출처 표의 개별 행은 아직 "제작 방식 미확인"이다. 디자인 담당에게 확인해 두 곳을 일치시켜야 한다.
- **담당**: 개발(문서 통합) / 디자인(원본 문서 v2.0·BGM 생성)

### 2026-08-10 (D-0) | 보스 체력 40% 하향 (4000 → 2400)

- **도구**: Claude Code (Opus 5) / Unity MCP (SerializedObject 주입)
- **작업**: Boss 프리팹 Health.maxHp를 4000 → 2400으로 낮췄다. 실효 체력은 시대별로 4000/4800/5800/7000 → 2400/2880/3480/4200이 된다.
- **프롬프트 원문**: "보스 체력이 너무 높아서 잡기 너무 힘드네 보스 체력을 40% 낮춰줘"
- **설계 판단과 근거**: 체력은 `프리팹 maxHp × EraManager.bossHpMultiplier`로 정해진다. **EraManager 쪽 배율 4개를 각각 0.6배 하는 안을 버리고 프리팹의 기준값 하나만 내렸다** — 배율을 건드리면 시대 진행에 따른 난이도 곡선(1 / 1.2 / 1.45 / 1.75)까지 손대는 셈이고, 값 4개가 코드 기본값과 씬 직렬화 값 양쪽에 있어 어긋나기 쉽다. 기준값 하나면 곡선은 그대로 두고 전체만 내려간다.
- **씬/프리팹 직렬화 함정을 먼저 확인했다**: EraManager의 배율은 SampleScene.unity에 이미 직렬화돼 있어(1 / 1.2 / 1.45 / 1.75) **코드 기본값을 고쳐도 런타임에 반영되지 않는다**. 이번엔 프리팹 경로를 택해 이 문제를 피했고, 어차피 애셋 파일이라 하드 룰대로 외부 편집 대신 `SerializedObject` → `ApplyModifiedPropertiesWithoutUndo` → `SaveAssets`로 주입했다.
- **검증 방법**: 주입 후 Boss.prefab 원문에서 `maxHp: 2400` 확인(추정 아님), 실행 로그 `Boss maxHp [4000] -> [2400]`, 콘솔 에러 0건(경고 1건은 무관한 랭킹 요청 타임아웃).
- **AI 산출물 vs 사용자 개입**: 수치 결정과 주입은 AI. 하향 폭 40%는 사용자 지시. 실제 체감 난이도는 플레이 검증 미실시 — 사용자 확인 필요.
- **담당**: 개발

### 2026-08-10 (D-0) | 타이틀 화면에 인게임 HUD(웨이브·레벨)가 겹쳐 보이던 문제

- **도구**: Claude Code (Opus 5) / Unity MCP (계층 덤프, SerializedObject 주입, 플레이 모드 스크린샷 2회)
- **작업**: TitleController에 `gameplayHudRoots` 배열을 추가해 Awake에서 끄고 StartRun()에서 켠다. 씬에서 WavePanel·LevelBadge를 연결했다.
- **프롬프트 원문**: "메인 화면에서 lv표시랑 웨이브 hud가 보이네 메인 ui 화면에서는 안보이게 해줘"
- **원인은 짐작이 아니라 계층 순서였다**: HUD_Canvas 자식 순서를 덤프해 보니 WavePanel·LevelBadge가 **TitlePanel보다 뒤에** 있었다. uGUI는 형제 순서가 곧 그리는 순서라 타이틀 오버레이 위에 얹힌다. HPBar·EXPBar는 TitlePanel보다 앞이라 같은 시각에 켜져 있는데도 가려져 안 보인다 — 사용자가 이 둘만 지목한 이유가 여기서 설명된다.
- **형제 순서를 바꾸지 않고 켜고 끄는 쪽을 택했다**: TitlePanel을 맨 뒤로 옮기면 이번 건은 해결되지만 GameOverPanel·PausePanel·ClassPanel과의 상대 순서까지 같이 흔들린다. D-0에 레이어 순서를 재배열할 이유가 없다.
- **직업 선택 화면도 같이 덮인다**: 끄는 시점을 Awake, 켜는 시점을 StartRun()으로 잡아 타이틀→직업 선택 구간 전체가 한 번에 처리된다(HandleStartPressed가 아니라 StartRun에 넣은 이유).
- **검증 방법**: 플레이 모드에서 activeSelf 실측(타이틀 False/False → StartRun 후 True/True), `ScreenCapture` 게임 뷰 캡처 2장으로 타이틀이 깨끗한 것과 시작 후 WAVE 1·LV 1이 되돌아오는 것을 눈으로 확인. 콘솔 에러 0건.
- **남은 것**: GameOverPanel도 WavePanel보다 앞이라 게임오버 창 위에 웨이브 HUD가 그대로 뜬다. 이번 요청 범위가 아니라 건드리지 않았다.
- **담당**: 개발

### 2026-08-10 (D-0) | 아트 생성 도구 확정 — 출처 표의 "제작 방식 미확인" 전량 해소

- **도구**: Claude Code (Opus 5)
- **작업**: 바로 앞 항목에서 남겨 뒀던 "몹·보스·플레이어·무기·장판·배경 아트의 제작 방식 미확인"을 사용자 확인으로 확정했다. 정지 원화는 전부 **Codex — GPT-5.6 Sol (reasoning high)**, 걷기·부유·발사·휘두르기 프레임은 **Higgsfield — Seedance 2.0 Fast** 4초 영상에서 선별한 것이다. BGM은 앞서 확정한 대로 **Suno (Pro 구독)**.
- **프롬프트 원문**: "아니 디자이너는 codex gpt-5.6 sol high와 힉스필드 seedance 2.0 fast로 전부 만든거야 추가해줘"
- **고친 곳**: 외부 에셋 출처 표 **6행**의 "제작 방식 미확인 — 제출 전 확인 필요"를 전부 실제 도구·모델명으로 교체했고, `AI 생성 에셋` 표와 기술서(`docs/시간관리국_AI_활용_기술서.md`) 02·04·08.5·09절을 같은 값으로 맞췄다. 기술서에서 일반명이던 "ImageGen"은 Codex 세션 안의 이미지 생성 기능임을 명시하는 문장으로 바꿨다.
- **왜 중요한가**: 제출물 ④의 요건이 "외부 에셋 / 오픈소스 출처"인데, **팀이 만든 것과 외부에서 받은 것을 구분하고 생성형 AI라면 모델명을 적는 것**이 그 요건의 실질이다. 이제 프로젝트의 모든 자산이 (1) CC0 외부 에셋 2종 + OFL 폰트 1종, (2) 팀 자체 생성(Codex / Seedance / Suno), (3) 코드가 런타임에 만드는 데이터(효과음 16종·이펙트 텍스처 6종) 셋 중 하나로 분류된다.
- **남은 흠**: 출처 표에 각 자산의 **생성 일시별 세부 프롬프트**는 없다(에셋별 README에 있다는 디자인 담당 기록에 의존). 심사에서 원문을 요구하면 그쪽을 첨부해야 한다.
- **담당**: 개발(문서 갱신) / 디자인(도구 정보 제공)

### 2026-08-10 (D-0) | 낱장 자산도 영상 단계를 거친다는 사실 확인 — 파이프라인이 하나로 정리됨

- **도구**: Claude Code (Opus 5)
- **작업**: 앞 항목에서 EXP 구슬·장판 2종·메인 배경을 "정지 원화만, 애니메이션 없음"으로 적었는데 사용자 확인 결과 **이것들도 Seedance 4초 영상을 거쳐 프레임을 선별한 것**이었다. 출처 표 3행을 정정하고, 기술서에 `4.7-A 낱장으로 쓸 자산도 영상 단계를 거친다` 절을 새로 넣었다.
- **프롬프트 원문**: (제가 "혹시 이것들도 Seedance를 거쳤다면 말씀해 주세요"라고 물은 데 대해) "거쳤어"
- **왜 문서가 좋아졌나**: 정정 전에는 자산 종류마다 파이프라인이 갈라지는 것처럼 읽혔는데, 실제로는 **모든 시각 자산이 예외 없이 같은 한 줄**을 지난다 — `Codex 정지 원화 → Seedance 4초 영상 → 97프레임 전개 → 사람이 선별 → 크로마키·알파 후처리 → 수치 검증 → Unity`. 낱장 자산은 마지막에 고르는 프레임 수가 1일 뿐이다. 기술서의 "AI 출력은 완성품이 아니라 후보 데이터다"라는 원칙이 **예외 없이 적용됐다는 증거**가 되므로 오히려 설명이 강해졌다.
- **왜 그렇게 하는지도 적었다**: 정지 생성 한 장은 다시 뽑을 때마다 형태가 흔들리지만 4초 영상은 같은 정체성으로 97개 변주를 한 번에 주므로 가장 안정된 프레임을 고를 수 있고, 프레임마다 조명·발광이 미세하게 달라 **4종 바닥색 위에서 가장 잘 읽히는 순간**을 고를 수 있으며, 이미 검증된 후처리 절차를 그대로 재사용할 수 있다.
- **내가 틀렸던 지점**: "낱장 = 애니메이션 불필요 = 영상 단계 불필요"라고 **추론으로 단정**했다. 아트 파이프라인은 최종 산출물 형태가 아니라 품질 안정화 방법으로 설계될 수 있는데 그 가능성을 빼고 적었다. 확인 없이 문서에 단정을 넣지 말 것.
- **담당**: 개발(문서 갱신) / 디자인(파이프라인 정보 제공)

### 2026-08-10 (D-0) | 게임 소개 및 설명 문서 작성 (제출물 ③)

- **도구**: Claude Code (Opus 5) / Unity MCP (씬 직렬화 값 실측)
- **작업**: `docs/시간관리국_게임소개.md` 신규. 요건 5가지(제목·한 줄 소개 / 목표·조작·종료 조건 / 실행 방법 / 플레이 링크 / 영상 링크)에 대응표를 붙이고, 처음 하는 심사자가 5분 안에 판을 굴릴 수 있게 팁·화면 안내까지 넣었다. 플레이 링크는 신규 도메인 `https://timecore.chohyunchang.com`.
- **프롬프트 원문**: "게임 제목 및 한 줄 소개 / 게임 방법: 목표·조작·종료 조건 포함 / 실행 방법 / 플레이 링크 또는 설치 방법 / 플레이 영상 링크 이제 이거 만들어줘 플레이 링크는 https://timecore.chohyunchang.com 이걸로 넣어주고 영상링크는 내가 나중에 넣을게 칸만 만들어줘"
- **README를 베끼지 않고 씬에서 실측했다**: 기존 `README.md`가 **증강 8종**으로 적혀 있었으나 실제로는 **23종**(`AugmentData` 에셋 전수 조회)이었고, 웨이브도 코드 기본값 25초가 아니라 씬 직렬화 값 **15초 × 4웨이브 → 5웨이브에 보스**였다. 플레이어 최대 체력 100·이동속도 6, 직업 카드 문구, 결과 화면 제목(`시간선 붕괴` / `역사 복구 완료`)과 표시 항목도 전부 씬에서 직접 읽어 적었다. **문서용 수치를 다른 문서에서 옮기면 옛 값이 그대로 굳는다.**
- **종료 조건을 두 갈래로 명시**: 요건이 "종료 조건 포함"이라 사망(`시간선 붕괴` — 생존 시간·처치·레벨·도달 웨이브·도달 시대)과 클리어(`역사 복구 완료` — 생존 시간·처치·레벨·최종 웨이브)를 각각 표시 항목까지 적었다. 자동 회복이 없고 회복 코어가 유일한 회복 수단이라는 점도 종료 조건 절에 붙였다 — 이게 없으면 "왜 죽었는지"가 설명되지 않는다.
- **브라우저 실행에서만 생기는 안내를 넣었다**: 첫 로딩 33MB, WebGL 2.0 필요, 키보드가 필요해 모바일 미지원, **소리는 첫 클릭 이후부터 재생**(브라우저 자동재생 차단). 마지막 항목은 심사자가 "소리가 안 난다"로 오해할 여지를 없애려는 것이다.
- **발견한 불일치(미수정)**: 직업 선택 화면의 BLADE 카드 문구가 `주위 적 회전 공격 · 반경 2.4`인데, 근접무기는 이미 홀을 **휘두르는** 방식으로 바뀌었다(회전 장판이 아니다). 문서에는 실제 동작대로 적었고 카드 문구 수정은 사용자 판단으로 남겼다.
- **담당**: 개발

### 2026-08-10 (D-0) | 커밋·푸시·WebGL 빌드·Vercel 배포 (deploy.bat이 옛 빌드를 올리던 문제 발견)

- **도구**: Claude Code (Opus 5) / Unity MCP (BuildPipeline) / Vercel CLI 58.1.0
- **작업**: 밀려 있던 작업 전부를 커밋 2개(docs / 기능·아트·밸런스)로 정리해 origin/main에 푸시(총 8커밋), WebGL 재빌드 후 프로덕션 배포.
- **프롬프트 원문**: "이제 커밋 푸시 빌드 배포 해줘"
- **씬 파일 때문에 커밋을 잘게 못 쪼갰다**: BGM·장판 아트·타이틀 아트·HUD 수정이 전부 SampleScene.unity 한 파일을 함께 건드려 기능별 분리가 불가능했다. 분리 가능한 docs만 떼고 나머지는 한 덩어리로 갔다 — 억지로 쪼개면 각 커밋이 빌드되지 않는다.
- **빌드는 "failed"로 보고됐지만 성공이었다**: CLAUDE.md에 적어 둔 그대로 Sentis(com.unity.ai.inference) 셰이더 경고 때문이다. 콘솔 에러 0건 + Build.data.br 재생성(22:50, 27.9MB → 32.7MB)으로 판정했다.
- **deploy.bat이 옛 빌드를 배포하고 있었다 (실측으로 발견)**: 배포 후 라이브 파일 크기가 27,937,162B — 새 빌드(32,650,577B)가 아니라 **21:06 빌드**였다. `rmdir /S /Q dist` + `xcopy` 단계가 먹지 않아 dist/Build가 통째로 옛 파일로 남아 있었다(타임스탬프 21:06 그대로). Copy-Item으로 직접 갱신해 재배포했고 라이브 크기가 32,650,577B로 일치하는 것을 확인했다. **배포 성공 메시지만 믿으면 안 된다는 사례** — vercel은 자기가 받은 걸 정상 배포했을 뿐이다.
- **용량 경고**: BGM 4곡(원본 6.4MB) 추가로 data.br이 4.7MB 늘었다(27.9 → 32.7MB). WebGL에서 용량 = 로딩 시간이라 저사양 브라우저 첫 로딩이 그만큼 길어진다.
- **담당**: 개발
